...
const InvoicePage = (props) => {
    ...
    const loadInvoicePage = async () => {
        ...
        if (snap.exists()) {
            ...
            setIFooter(snap.data().ifooter ? snap.data().ifooter : userData.ifooter);
            saveFooterNote(snap.data().ifooter ? snap.data().ifooter : userData.ifooter);
            if (snap.data().client) {
                setClientData(snap.data().client)
                setIsClientSelected(true);
            }
        ...
        if (planConfig[userData.planAnalysis.plan].features.emailInvoice) {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    updateRecentActivity(`Invoice mailed to ${clientData.name}.`);
                    setLoading(false);
                } else {
        ...
        <>
            ...
            {showRecordModal && (
                <div className="fixed inset-0 z-40 px-5 flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden max-h-[80vh]">
                        {/* Header */}
                        <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between">
                            <h3 className="text-sm font-semibold text-[var(--themeBlack)] flex items-center gap-3">
                                <i className="fa-solid fa-money-check-dollar text-[var(--primaryPanel)] text-sm"></i>
                                Payment History
                            </h3>
                            <button onClick={() => { setShowRecordModal(false) }} className="text-gray-900 cursor-pointer hover:text-black text-2xl">&times;</button>
                        </div>
                        {/* Body */}
                        <div className="p-5 space-y-3 overflow-y-auto max-h-[calc(80vh-133px)]">
                            {paymentHistory && paymentHistory?.length > 0 ? (
                                paymentHistory.map((entry, index) => (
                                    <div
                                        key={index}
                                        className="border border-gray-200 rounded-lg px-4 py-3 bg-gray-50 hover:bg-gray-50 transition-all flex justify-between items-start group"
                                    >
                                        <div>
                                            <p className="text-sm font-medium text-[var(--themeBlack)]">
                                                {productDataArray ? currencyShow[productDataArray[0].currency] : ''} {Number(entry.amount).toFixed(2)}
                                                <span className="text-xs text-gray-500 ml-2">({entry.method})</span>
                                            </p>
                    ...
                    {showNewRecordModal && (
                        <div className="fixed inset-0 z-45 px-5 flex items-center justify-center bg-black/50">
                            <div className="bg-white rounded-xl shadow-lg w-full max-w-sm overflow-hidden max-h-[80vh]">
                                {/* Header */}
                                <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between">
                                    <h3 className="text-sm font-semibold text-[var(--themeBlack)] flex items-center gap-3">
                                    ...
                                    <button onClick={(onClose) => { setShowNewRecordModal(false) }} className="text-gray-900 cursor-pointer hover:text-black text-2xl">&times;</button>
                                </div>
                                {/* Form */}
                                <div className="px-6 pt-5 pb-4 space-y-4 text-sm text-gray-700 max-h-[calc(80vh-56px)] overflow-y-auto">
                                    <div>
                                ...
                                {/* Footer */}
                                <div className="py-3 px-5 border-t border-gray-200 flex justify-end">
                                    <button
                                        onClick={() => { savePayment() }}
                                        disabled={loading}
                    ...
                    <div className='py-6 px-4 md:px-8'>
                            ...
                            <div className='flex-1 mx-1 md:mx-0'>
                                <div className='flex justify-start items-center'>
                                    <div className='flex gap-2 justify-start items-center'>
                                        <h1 className='text-xl font-semibold'>#{formData.invoiceNo} Invoice</h1>
                                        {invoiceStatus == 'Draft' && (
                                ...
                                <h3 className='text-sm font-medium text-gray-500'>create and edit invoices within seconds!</h3>
                            </div>
                            <div className='flex gap-3 justify-between md:justify-end'>
                                <div>
                                    <button onClick={() => { showPreview() }} className='text-[14px] py-2 px-5 bg-gray-100 text-[var(--themeBlack)] font-semibold rounded-md cursor-pointer hover:bg-gray-300 active:bg-gray-300'>PREVIEW</button>
                                </div>
                                <div>
                                    <button onClick={() => { openPDFInNewTab() }} className='text-[14px] py-2 px-5 bg-gray-100 text-[var(--themeBlack)] font-semibold rounded-md cursor-pointer hover:bg-gray-300 active:bg-gray-300'>PDF</button>
                                    <button onClick={() => { sendMailToClient() }} className='text-[14px] ml-2 py-2 px-5 bg-[var(--primaryPanel)] text-white font-semibold rounded-md cursor-pointer hover:bg-[var(--greenDarkPanel)] active:bg-[var(--greenDarkPanel)]'>EMAIL <i className='fa-solid fa-angle-right ml-2'></i></button>
                                </div>
                            </div>
                        </div>
                        <div className='mt-2 sm:mt-5 mb-5 max-w-full flex flex-col lg:flex-row gap-4'>
                            <div className='flex-1 flex flex-col gap-4'>
                                {/* invoice details */}
                                {/* bg-[#F6F7F9] */}
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    {/* Header */}
                                    <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between border-b border-[var(--greenLightPanel)]">
                                        <h3 className="text-sm font-semibold text-[var(--themeBlack)] flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col md:flex-row gap-4">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full text-md md:text-sm px-3 py-2 border-2 border-gray-400 rounded-md focus:border-[var(--primaryPanel)] outline-none text-[var(--themeBlack)]"
                                                />
                                            </div>
                                            {/* Invoice Date */}
                                            <div className="flex-1">
                                ...
                                <div className='flex flex-col md:flex-row gap-4'>
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                        ...
                                        <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between border-b border-[var(--greenLightPanel)]">
                                            ...
                                            <Link href='/portal/settings'><button
                                                className="text-xs font-semibold cursor-pointer text-black hover:underline transition duration-200 flex items-center gap-1"
                                            >
                                        ...
                                        <div className="px-6 pt-5 pb-6 flex flex-col items-start gap-4">
                                            ...
                                            <div>
                                                <div
                                                    className="w-60 h-15 border border-dashed border-gray-300 rounded-md flex items-center justify-center bg-white hover:border-[var(--primaryPanel)] cursor-pointer transition"
                                            ...
                                            <div className="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 w-full flex flex-col gap-1 text-sm text-gray-700">
                                                <p className="text-base font-semibold text-[var(--themeBlack)]">{businessData.name || '-'}</p>
                                                <div className="flex gap-3 items-start">
                                                    <i className="fa-solid fa-envelope text-[var(--primaryPanel)] mt-[3px] text-xs"></i>
                                                    <span className="text-sm text-gray-600">{businessData.mail || '—'}</span>
                                                </div>
                                                <div className="flex gap-3 items-start">
                                                    <i className="fa-solid fa-phone text-[var(--primaryPanel)] mt-[3px] text-xs"></i>
                                                    <span className="text-sm text-gray-600">{businessData.phone || '—'}</span>
                                                </div>
                                                <div className="flex gap-3 items-start">
                                    ...
                                    {/* client details */}
                                    <div className="bg-white border border-[var(--greenLightPanel)] rounded-xl shadow-sm w-full">
                                        <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between border-b border-[var(--greenLightPanel)]">
                                            <h3 className="text-sm font-semibold text-[var(--themeBlack)] flex items-center gap-4">
                                        ...
                                        <div className="px-4 md:px-6 pt-5 pb-6">
                                            <div className="mb-4 w-full max-w-sm">
                                                <p className="text-sm font-semibold text-gray-500 mb-[1px]">
                                            ...
                                            {isClientSelected && (
                                                <div className="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 shadow-sm text-sm text-gray-800 space-y-1">
                                                    <div className="font-semibold text-[15px] text-[var(--themeBlack)] leading-tight">
                                                        {clientData.name}
                                                    </div>
                                                    {clientData.mail && (
                                                        <div className="flex items-center text-gray-700 mt-1">
                                                            <i className="fa-solid fa-envelope text-[var(--primaryPanel)] text-xs mr-2"></i>
                                                            <span className="truncate">{clientData.mail}</span>
                                                        </div>
                                                    )}
                                                    {clientData.phone && (
                                                    ...
                                                    {clientData.address && (
                                                        <div className="flex items-start text-gray-700">
                                                            ...
                                                            <span className="truncate">{clientData.address}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                ...
                                <div className="bg-white rounded-xl border border-[var(--greenLightPanel)] shadow-sm w-full">
                                    <div className="bg-[var(--greenLightestPanel)] rounded-t-xl px-7 py-3 flex items-center justify-between border-b border-[var(--greenLightPanel)]">
                                        <h3 className="text-sm font-semibold text-[var(--themeBlack)] flex items-center gap-4">
                                    ...
                                    <div className="px-4 md:px-6 pt-5 pb-6">
                                        ...
                                        {isProductSelected && (
                                            <div className="relative overflow-x-auto overflow-y-hidden rounded-md shadow-sm -mx-2 sm:mx-0">
                                                <table className="min-w-[95vw] sm:min-w-full text-sm text-left text-gray-800">
                                                    <thead className="bg-gray-100 text-gray-700 text-[13px] font-semibold">
                                                        <tr>
                                                    ...
                                                    <tbody>
                                                        {productDataArray.map((product) => (
                                                            <tr
                                                                key={product.id}
                                                                className="hover:bg-gray-50 even:bg-gray-50 transition border-t border-gray-200"
                                                            >
                                                                <td className="py-3 px-3 w-full">
                                                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                                                                        <div className="flex flex-col sm:max-w-[250px]">
                                                                            <span className="font-semibold text-[12px] sm:text-[13.5px] text-[var(--themeBlack)] leading-snug">
                                                                                {product.name}
                                                                            </span>
                                                                            {product.description && (
                                                                                <span className="text-xs text-gray-500 leading-tight mt-[1px]">
                                                                                    {product.description}
                                                                                </span>
                                                                            )}
                                                                        </div>
                                                                        <div
                                                                            ...
                                                                            className="shrink-0 max-w-fit px-3 py-[3px] rounded-md bg-red-50 hover:bg-red-100 text-red-700 text-[10px] font-medium border border-red-100 cursor-pointer transition whitespace-nowrap"
                                                                        >
                                                                            REMOVE
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <td className="py-3 px-3 text-right">
                                                                    <input
                                                                        ...
                                                                        className="w-11 text-center py-[4px] border border-[var(--grey66)] rounded-md focus:border-[var(--primaryPanel)] text-sm text-[var(--themeBlack)]"
                                                                    />
                                                                </td>
                                        ...
                                        {/* Summary */}
                                        {currentCurrency && (
                                            <div className="pt-4 md:px-3 space-y-2 text-sm text-gray-700">
                                                <div className="flex justify-between font-semibold">
                                                    <span className="text-gray-500">Pricing Total</span>
                                                    <span className="font-semibold text-[var(--themeBlack)]">
                                                        {currencyShow[currentCurrency]} {Number(productSubtotal).toFixed(2)}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between font-semibold">
                                                    <span className="text-gray-500">{(taxData.taxType == 'On Total' || taxData.taxType == 'Deducted')
                                                ...
                                                <div className="flex justify-between items-end text-base md:text-lg font-semibold text-[var(--themeBlack)]">
                                                    <span>Amount Due</span>
                                                    <span className='text-base md:text-lg  text-[var(--themeBlack)]'>
                                ...
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    <div className="px-4 md:px-8 pt-5 pb-6 flex flex-col gap-6">
                                        ...
                                        <div className="w-full">
                                            <p className="text-sm font-medium text-gray-700 mb-1">Invoice Footer Note</p>
                                            <textarea
                                                name="invoiceFooter"
                                                value={iFooter}
                                                onChange={(e) => setIFooter(e.target.value)}
                                                onBlur={(e) => saveFooterNote(e.target.value)}
                                                className="w-full rounded-md border-2 border-gray-400 text-md md:text-sm text-[var(--themeBlack)] px-3 py-2 outline-none focus:border-[var(--primaryPanel)] resize-none min-h-[90px]"
                                                placeholder="Write a thank you message, bank details, or any custom note..."
                                            />
                                        </div>
                                        {/* Signature */}
                                        <div className="w-full max-w-sm">
                                            <p className="text-sm font-medium text-gray-700 mb-2">Authorized Signature</p>
                                            <div
                                                className="w-full max-w-sm aspect-[5/2] border-2 border-dashed border-gray-300 rounded-md flex items-center justify-center cursor-pointer bg-white hover:border-[var(--primaryPanel)] transition"
                                            ...
                                            <div className="mt-3">
                                                ...
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                    className="block cursor-pointer w-full text-sm text-gray-700 file:mr-3 file:py-1 file:px-3 file:rounded-md file:border-0 file:bg-[var(--primaryPanel)] file:text-white hover:file:bg-[var(--primaryPanel)]"
                                                    ref={inputRefSign}
                            ...
                            <div className='w-full py-3 bg-[var(--greenLightestPanel)] text-black text-center rounded-md block lg:hidden text-sm font-semibold'>
                                Invoice Settings
                            </div>
                            <div className='w-full max-w-full lg:max-w-[300px] flex flex-col gap-4'>
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    <div className="px-5 sm:px-7 py-5 flex flex-col gap-1">
                                        {/* Live URL Tag */}
                                        <div className="inline-block bg-[var(--primaryPanel)] text-white text-xs font-semibold px-2 py-[4px] rounded w-max">
                                            LIVE URL
                                        </div>
                                        {/* Invoice URL */}
                                        <a
                                        ...
                                        <div className="flex gap-3 flex-wrap mt-2">
                                            <button
                                                ...
                                                data-tooltip-place="top"
                                                className="text-xs bg-gray-50 cursor-pointer font-medium px-3 py-1 border border-gray-300 rounded-md text-gray-700 hover:border-[var(--primaryPanel)] hover:text-[var(--primaryPanel)] transition"
                                            >
                                            ...
                                            <a
                                                ...
                                                data-tooltip-place="top"
                                            >
                                                <button className="text-xs bg-gray-50 cursor-pointer font-medium px-3 py-1 border border-gray-300 rounded-md text-gray-700 hover:border-blue-500 hover:text-blue-500 transition">
                                ...
                                <div onClick={() => setShowRecordModal(true)} className='w-full px-4 py-3 text-center text-white cursor-pointer rounded-md bg-[var(--primaryPanel)] hover:bg-[var(--greenDarkPanel)] hover:text-white text-sm font-semibold transition'>
                                ...
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    {/* Content */}
                                    <div className="px-6 py-5 flex flex-col gap-3 text-[var(--themeBlack)] text-sm">
                                        {recentActivity ? (
                                            recentActivity.map((value, index) => (
                                                index < 3 && (<InvoiceActivityBar heading={value.message} date={value.date} time={value.time} key={index} />)
                                            ))
                                        ) : (
                                            <p className='text-sm text-gray-500 fot-semibold'>Loading Recent Activities...</p>
                                        )}
                                        {recentActivity && recentActivity?.length > 3 && (
                                            <button onClick={() => { setShowActivityModal(true) }} className="text-xs mt-1 bg-gray-50 cursor-pointer font-medium px-3 py-1 border border-gray-300 rounded-md text-gray-700 hover:border-blue-500 hover:text-blue-500 transition">
                                ...
                                {/* <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    <div className="px-5 sm:px-7 pt-5 pb-7 flex flex-col gap-4 text-[var(--themeBlack)] text-sm">
                                        ...
                                        <button
                                            className="text-sm bg-gray-50 cursor-pointer font-semibold px-3 py-3 border border-[var(--primaryPanel)] rounded-md text-[var(--primaryPanel)] hover:border-[var(--greenDarkPanel)] hover:text-[var(--greenDarkPanel)] transition"
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <label className="font-medium text-gray-700 flex items-center gap-2">
                                                ...
                                                <span
                                                    className="bg-gray-200 text-gray-600 hover:bg-gray-300 transition rounded-full text-xs px-2 py-[1px] cursor-pointer"
                                                    data-tooltip-id="my-tooltip"
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        className="w-full text-right py-2 pr-10 pl-2 rounded-md border border-[var(--grey66)] focus:border-[var(--primaryPanel)] text-sm placeholder:text-gray-400 text-[var(--themeBlack)]"
                                                        placeholder="0"
                                                    />
                                ...
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    {/* Content */}
                                    <div className="px-5 sm:px-7 py-5 flex flex-col gap-4 text-sm text-[var(--themeBlack)]">
                                        {/* Invoice Theme */}
                                        <div>
                                ...
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    {/* Content */}
                                    <div className="px-5 sm:px-7 pt-5 pb-7 flex flex-col gap-4 text-sm text-[var(--themeBlack)]">
                                        {/* Tax Type */}
                                        <div>
                                        ...
                                        {(taxData.taxType === 'On Total' || taxData.taxType === 'Deducted') && (
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <div className="flex-1">
                                                    <label className="block text-gray-700 font-medium mb-1">
                                                        Tax Label <span className="text-sm text-gray-500">(i.e. Tax, GST)</span>
                                                    </label>
                                                    <input
                                                        type="text"
                                                        name="taxlabel"
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            className="pr-10 text-right w-full py-3 pl-2 md:py-2 box-border outline-0 border-2 border-[var(--grey66)] text-[var(--themeBlack)] placeholder:text-gray-500 rounded-md focus:border-[var(--primaryPanel)]"
                                                            placeholder="0"
                                                        />
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-3">
                                                        <label htmlFor="inclusiveTax" className="text-sm font-medium text-gray-700 cursor-pointer" data-tooltip-id="my-tooltip" data-tooltip-content="Does the price include tax?" data-tooltip-place="top">
                                                            Inclusive?
                                                        </label>
                                                        <input
                                                            type="checkbox"
                                ...
                                <div className="bg-white rounded-xl shadow-sm w-full border border-[var(--greenLightPanel)]">
                                    ...
                                    {/* Content */}
                                    <div className="px-5 sm:px-7 pt-5 pb-7 flex flex-col gap-5 text-sm text-[var(--themeBlack)]">
                                        {/* Discount Type */}
                                        <div>
                                            <label className="block text-gray-700 font-medium mb-1">
