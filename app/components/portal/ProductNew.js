...
const ProductNew = () => {
    ...
    const submitHandler = async (e) => {
        ...
        const docCollection = collection(db, 'happyuser', currentUser.uid, 'happyproduct');
        const snap = await addDoc(docCollection, savingValue);
        const docClient = doc(db, 'happyuser', currentUser.uid);
        const newProductArray = userData.products ? userData.products : [];
        newProductArray.push(name);
        const newProductPriceArray = userData.productprice ? userData.productprice : [];
        newProductPriceArray.push(price);
        ...
        await updateDoc(docClient, { totalproduct: userData.totalproduct + 1, activity: recentActivity, products: newProductArray, productprice: newProductPriceArray })
            ...
            <div className="w-[100vw] h-[100vh] overflow-y-auto bg-[var(--themeBlackTrans)] fixed flex justify-center items-start p-6">
                <div className="rounded-xl bg-[var(--themeWhite)] w-full max-w-[450px]">
                    <div className='flex justify-center items-center rounded-tl-lg rounded-tr-lg bg-[var(--greenLightestPanel)] py-3 px-6'>
                        <h3 className='font-semibold text-black flex-1'>Add Product</h3>
                        <i className='fa-solid fa-close text-xl cursor-pointer px-2 py-1' onClick={() => { setIsNewProduct(false) }}></i>
                    </div>
